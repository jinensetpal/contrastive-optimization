...
\begin{proof} Using the definition of HiResCAMs, ContrastiveCAMs and the elementwise difference setup from \ref{classprobdiff}, we have:
	...
	\end{align}
	As an aside: we note that when $i = c$, the contrastive map is $\bm{0} \in \mathbb{R}^{D_1 \times D_2}$, and instead of $|C| - 1$ contrastive maps we now obtain $|C|$ contrastive maps. \newline \\
...
\subsubsection{ACE-Divergence Fomulation}
...
\subsection{Training Invariance to Spurious Correlation}

One significant concern uncovered both theoretically from \ref{ablatedce} and empirically from \ref{contr-cam} was that the standard classification pipeline motivates learning spurious correlations between the model, provided the spurious information is able to improve accuracy. This motivates learning distribution-specific correlations, which worsens it's ability to generalize. As an example
...
\begin{figure}[H]
	...
	\caption{Left: contrastive activation maps from the default training run. Middle: contrastive activations maps from the Ablated CE+KLD training run. Right: contrastive activation maps from the Ablated CE training run.}
\end{figure}
