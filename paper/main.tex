...
\begin{gather}
	\max_{\theta} \sum^{D_1,D_2}_{j,k}\tilde{\mathcal{\bm{A}}}_{i,j,k}^{\text{contrastive}}\ \forall i \in \mathbb{Z}_{+}(|c| - 1)
\end{gather}
...
\begin{gather}
	...
	\begin{split}
		\mathcal{\bm{J}}(\theta, \mathcal{D}) := &\ \lambda_1 D_{KL}(\sigma_{\text{softmax}}(\lambda_2 \mathcal{\bm{A}}^{\text{contrastive}})\ ||\ \sigma_{\text{softmax}}(\lambda_3 h)) \\
		&+ \frac{\lambda_4}{\sum^{D_1,D_2}_{i,j}\mathbbm{1}(h_{i,j} = 0)} ||\mathcal{\bm{A}}^{\text{background}}||^2_F \\
		&- \frac{\lambda_5}{\sum^{D_1,D_2}_{i,j}\mathbbm{1}(h_{i,j} = 1)} \sum^{|c|-1,D_1,D_2}_{c,i,j} \mathcal{\bm{A}}^{\text{foreground}}_{c,i,j}
	\end{split}
\end{gather}
