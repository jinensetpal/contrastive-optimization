...
\begin{gather}
	\max_{\theta} s_c + b_c - s_{c'} - b_{c'} \equiv \max_{\theta} \tilde{\mathcal{\bm{A}}}_{c}^{\text{HiResCAM}} + b_c - \tilde{\mathcal{\bm{A}}}_{c'}^{\text{HiResCAM}} - b_{c'}
\end{gather}
Ignoring the bias terms, our new optimization objective is:
\begin{gather}
	\max_{\theta} \mathcal{\bm{A}}^{\text{contrastive}} := \tilde{\mathcal{\bm{A}}}_{c}^{\text{HiResCAM}} - \tilde{\mathcal{\bm{A}}}_{c'}^{\text{HiResCAM}}
\end{gather}
...
\begin{gather}
	\mathcal{\bm{A}}^{\text{contrastive}} := \{\tilde{\mathcal{\bm{A}}}_{c_t}^{\text{HiResCAM}} - \tilde{\mathcal{\bm{A}}}_{c'}^{\text{HiResCAM}}\}^{|c|-1}_{c_{t'} \in c - c_t}
\end{gather}
...
\begin{gather}
	\max_{\theta} \tilde{\mathcal{\bm{A}}}_{i}^{\text{contrastive}}\ \forall i \in \mathbb{N}(|c| - 1)
\end{gather}
...
Segmentations maps describes the region over which the object is present. We can leverage this information to define a constrastive cost function as follows:
\begin{gather}
	...
	\mathcal{\bm{A}}^{\text{background}}_{c,i,j} := \begin{cases}
		\mathcal{\bm{A}}^{\text{contrastive}}_{c,i,j} & h_{i,j} = 0 \\
		0 & \text{otherwise}
	\end{cases} \\
	\mathcal{\bm{A}}^{\text{foreground}}_{c,i,j} := \begin{cases}
		0 & h_{i,j} = 0 \\
		\mathcal{\bm{A}}^{\text{contrastive}}_{c,i,j} & \text{otherwise}
	\end{cases} \\
	...
	\mathcal{\bm{J}}(\theta, \mathcal{D}) := D_{KL}(\sigma_{\text{softmax}}(\lambda_1 \mathcal{\bm{A}}^{\text{contrastive}})\ ||\ \sigma_{\text{softmax}}(\lambda_2 \cdot h)) + \frac{\lambda_3}{D_1 D_2} ||\mathcal{\bm{A}}^{\text{background}}||^2_2 - \frac{1}{D_1 D_2} \sum^{|c|-1,D_1,D_2}_{c,i,j} \mathcal{\bm{A}}^{\text{foreground}}_{c,i,j}
\end{gather}
...
$D_{KL}$ is modified per (10), such that the likelihood comparator sets point probability to 1 if it is part of the foreground. While this violates the probability distribution definition, it motivates the optimizer to maximize the foreground activation. During training, observing pointwise divergence revealed that the model was penalized for `over-activating' the foreground, which this modification corrects. In addition, it also averages the divergence from the target segmenetation map to each contrastive activation map.
...
For binary classification, the model was trained for $150$ epochs with the Adam optimizer with a learning rate of $10^{-3}$, a batch size of $32$ on a 32GB V100 on a randomly shuffled dataset using the original data splits. The total time for training was $\approx$1hr.
...
\begin{table}[h]
	...
	\begin{tabular}{c|ccc}
		...
		\textbf{Method}  & \textbf{Cross Entropy Loss}  & \textbf{Training Accuracy}   & \textbf{Validation Accuracy} \\
		\midrule
		Cross-Entropy & 0.000 & 3.0\% & 3.0\% \\
		Interpretable (Ours) & \bf 0.000 & \bf 92.0\% & \bf 50.0\% \\
		\bottomrule
	\end{tabular}
\end{table}
...
\paragraph{Understanding the Generalization Gap} While the model for the multiclass paradigm is able to fit the training dataset excellently -- achieving $\approx 92\%$ training accuracy -- the validation accuracy is significantly behind at $\approx 50\%$. This could imply memorization of samples, which was something that the approach was intended to mitigate. Further investigation could help support or disprove this result.
