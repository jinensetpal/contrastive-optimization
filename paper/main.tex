...
\end{gather}
This is equivalent to the original ERM optimization objective, with the difference being the resultant values, which is an activation map instead. If $\tilde{\mathcal{\bm{A}}}_{c}^{\text{HiResCAM}} - \tilde{\mathcal{\bm{A}}}_{c'}^{\text{HiResCAM}} > 0$, the classification is correct -- if not, the classification is incorrect. Ignoring the bias terms, our new optimization objective is:
\begin{gather}
	\max_{\theta} \tilde{\mathcal{\bm{A}}}_{c}^{\text{HiResCAM}} - \tilde{\mathcal{\bm{A}}}_{c'}^{\text{HiResCAM}}
\end{gather}
...
\begin{gather}
	...
	\end{cases} \\
	D_{KL}(P || Q) := P \cdot \log \frac{P'}{Q} \text{ where }
	P'_{i,j} := \begin{cases}
		1 & P_{i,j} \neq 0 \\
...
$D_{KL}$ is modified such that the likelihood comparator sets point probability to 1 if it is part of the foreground. While this violates the probability distribution definition, it motivates the optimizer to maximize the foreground activation. During training, observing pointwise divergence revealed that the model was penalized for `over-activating' the foreground, which this modification negates.
