...
class Dataset(torchvision.datasets.SBDataset):
    ...
    def __getitem__(self, idx):
        ...
        return pil_to_tensor(resize(X, const.IMAGE_SIZE, antialias=False)).to(self.device) / 255, (heatmap.to(self.device), y.to(self.device))
...
def get_generators():
    ...
    dataloaders = *[torch.utils.data.DataLoader(Dataset(const.DATA_DIR / 'sbd', mode='segmentation', image_set=split, device='cpu', download=not (const.DATA_DIR / 'sbd').exists()),
